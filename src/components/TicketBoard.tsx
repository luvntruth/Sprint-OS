...
export function TicketBoard({ state, setState, readOnly = false }: Props) {
  ...
  const publicFallbackTicket: Ticket = {
    ...
    stage,
    title: stage === 'prep' ? '사전 설문 응답과 문제 상황 예시 준비' : '내 카드의 다음 액션 실행',
    description:
      stage === 'prep'
        ? '첫 모임 전 설문에 답하고, 내가 다루고 싶은 현업 문제 상황 예시 1~2개를 준비합니다.'
        : '우리 카드에서 내 다음 액션을 확인하고, 다음 모임 전 작게 실행합니다.',
    owner: 'participant',
    status: 'todo',
    priority: 'high',
    ...
    <section className="panel ticket-board-page">
      <div className="screen-heading">
        <div>
          ...
          <h1>{readOnly ? '이번 주 할 일' : '운영 티켓 보드'}</h1>
          <p>{readOnly ? '참가자는 지금 단계에서 무엇을 하면 되는지만 확인합니다.' : '운영자(문턱장)는 단계별 티켓을 갱신하며 1기 운영을 관리합니다.'}</p>
        </div>
        <span className="screen-mode-pill">{stageMeta[stage].label}</span>
      </div>
      ...
      {readOnly ? (
        <div className="participant-task-summary">
          <span>이번 단계에서 참가자가 할 일</span>
          <strong>{completion.done}/{completion.total} 완료</strong>
          <p>{stageMeta[stage].purpose}</p>
        </div>
      ) : (
          ...
          <article className={`ticket-row status-${ticket.status}`} key={ticket.id}>
            ...
            {!readOnly && (
              <div className="ticket-controls">
                ...
                <label>담당<select value={ticket.owner} onChange={(event) => update(ticket.id, { owner: event.target.value as Ticket['owner'] })}><option value="operator">운영자</option><option value="participant">참가자</option><option value="ai">AI/로키</option></select></label>
              </div>
            )}
          </article>
        ))}
